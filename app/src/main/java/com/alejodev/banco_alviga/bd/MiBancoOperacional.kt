...
class MiBancoOperacional protected constructor(context: Context?) {
    ...
    fun login(c: Cliente): Cliente? {
        ...
        // Hemos echo una modificacion del codigo de anna en la que intentamos convertir el resultado a cliente para que no lance una excepcion
        // Si el cliente no existia me cerraba al app
    ...
    fun getCuentas(c: Cliente?): ArrayList<Cuenta>? {
    ...
    */
    ...
    fun añadirCliente(c: Cliente): Boolean{
        val resultado: Long? = miBD?.clienteDAO?.add(c)
        return resultado != null && resultado != -1L
    }
